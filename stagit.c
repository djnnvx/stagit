...
static char *license;
static char *readmefiles[] = {"HEAD:README", "HEAD:README.md", "HEAD:README.txt", "HEAD:readme", "HEAD:readme.txt"};
static char *readme;
...
size_t writeblobhtml(FILE *fp, const git_blob *blob) {
    ...
    fputs("<pre id=\"blob\" style=\"text-align: left; width: 33.33%; margin: 0 auto; white-space: pre-wrap; word-wrap: break-word; overflow-wrap: break-word\">\n", fp);
...
int main(int argc, char *argv[]) {
    ...
    if (readme) {
        ...
        if (r == 1) {
            ...
            if (md_html(s, len, process_output_md, fp, MD_FLAG_TABLES | MD_FLAG_TASKLISTS | MD_FLAG_PERMISSIVEEMAILAUTOLINKS | MD_FLAG_PERMISSIVEURLAUTOLINKS, 0))
                fprintf(stderr, "Error parsing markdown\n");
            fputs("</div>\n", fp);
        } else {
            fputs("<pre style=\"text-align: left; width: 33.33%; margin: 0 auto; white-space: pre-wrap; word-wrap: break-word; overflow-wrap: break-word\" id=\"readme\">", fp);
            xmlencode(fp, s, strlen(s));
            fputs("</pre>\n", fp);
        }
        writefooter(fp);
